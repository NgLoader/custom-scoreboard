...
import de.ngloader.scoreboard.api.ScoreboardTeamRule;
...
public class SharedTeam extends SharedBase<SharedTeam> implements ScoreboardTeam {
	...
	WrappedChatComponent displayNameWrapped = ComponentUtil.EMPTY_CHAT_COMPONENT;
	WrappedChatComponent prefixWrapped = ComponentUtil.EMPTY_CHAT_COMPONENT;
	WrappedChatComponent suffixWrapped = ComponentUtil.EMPTY_CHAT_COMPONENT;
	...
	protected void createUpdatePackets(List<PacketContainer> packets) {
		...
		if (!this.dirtJoinEntrys.isEmpty()) {
			packets.add(ScoreboardProtocol.joinTeamPacket(this, new HashSet<String>(this.dirtJoinEntrys)));
			this.dirtJoinEntrys.clear();
		}
		if (!this.dirtLeaveEntrys.isEmpty()) {
			packets.add(ScoreboardProtocol.leaveTeamPacket(this, new HashSet<String>(this.dirtLeaveEntrys)));
	...
	@Override
	public SharedTeam addEntry(Entity... entities) {
		for (Entity entity : entities) {
			this.addEntry(entity instanceof Player ? entity.getName() : entity.getUniqueId().toString());
		}
		return this;
	}

	@Override
	public SharedTeam addEntry(String... entries) {
	...
	@Override
	public SharedTeam removeEntry(Entity... entities) {
		for (Entity entity : entities) {
			this.removeEntry(entity instanceof Player ? entity.getName() : entity.getUniqueId().toString());
		}
		return this;
	}

	@Override
	public SharedTeam removeEntry(String... entries) {
		for (String entry : entries) {
